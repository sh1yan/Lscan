...
// PortDefaultProtocol 显示该端口号默认应用服务
func PortDefaultProtocol(port int) (result string) {
	result = "unknown"
	if name, ok := commonlist[port]; ok {
		return name
	} else if name, ok := detailedlist[port]; ok {
		return name
	}
	return result
}

// commonlist 默认端口协议简称列表
...
var detailedlist = map[int]string{
	...
	2498:  "ODN-CasTraq",
	2499:  "UniControl",
	2500:  "Resource Tracking systemapp server",
	2501:  "Resource Tracking systemapp client",
